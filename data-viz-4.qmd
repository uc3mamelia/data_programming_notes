...
![](images/clipboard-4282288537.png)
...
```{r}
p + facet_wrap(cyl ~ class)
```

By default, drop is true in facet_wrap. Facet_grid takes one factor and takes all the levels in that factor as rows. Takes the other variable and takes all the levels as columns and fixes all the combinations of both. Facet_wrap removes empty panels.

```{r}
p + facet_wrap(cyl ~ class, ncol=4, drop=FALSE)
```

The above would produce the same as facet_grid while using facet_wrap. Iñaki uses facet_grid as he finds it is simpler to see and understand as well as cleaner.

2.  `facet_grid()` has the same parameter you modified above. Why is it not taking any effect? When would it take effect?

```{r}
p + facet_grid(cyl ~ class, drop=TRUE)
```

Facet_grid has 'drop' as well, but it doesn't affect the data. It would only affect the whole column or row if it was empty.
...
p + facet_grid(class ~ cyl, margins = TRUE) 
```

Margins adds in the 'all' columns and rows, which collects all the data in the column or all the data in the row.
...
```{r}
p + aes(color = variable)
```

You could color the lines, but the series have different units. It is also difficult to compare because they have different y values.

```{r}
p + facet_grid(variable ~ .)
```

Faceting makes it better, but you still have the scale issue.

```{r}
p + facet_grid(variable ~ ., scales="free_y")
```

Free_y makes it even better.
...
p <- ggplot(economics_long) +
  aes(date, value) +
  geom_line()
p + facet_grid(variable ~ ., scales="free_y", space = "free_y")
```

This is a problem because we cannot compare them anymore. The ones that have small ranges get condensed.

```{r}
p <- ggplot(mpg) + aes(cty, model) + geom_point()
p + facet_grid(manufacturer ~ .)
```

This is a mess. It is putting all the models in there.

```{r}
p + facet_grid(manufacturer ~ ., scales = "free_y")
```

We only have the relevant data now, but we have a problem with a spacing.

```{r}
p + facet_grid(manufacturer ~ ., scales = "free_y", space = "free_y")
```

If "free_y" their height will be proportional to the length of the y scale.

If "free_x", their width with be proportional to the length of the of the x scale.

It only makes sense to free the axis with which you are faceting.
...
mpg2 <- mpg # creating a new variable for mpg
mpg2$cyl <- NULL  # removing column cyl from mpg2

p + facet_grid(cyl~.) +
  geom_point(data=mpg2, color="lightgray") +
  geom_point()
```
