...
If the result still looks a bit confusing, there are further options to customize `ggrepel`’s behavior. `geom_label_repel()` automatically adds the line if it determines that the label is too far away from the point.
...
```{r}
p + ggrepel::geom_label_repel(aes(label=model), annotation, fill="red")
```
...
Making the labels white and down
...
ggplot(mpg |> dplyr::count(class)) +
  aes(class, n, label=n) +
  geom_col() +
  geom_text(vjust = 1, nudge_y = -1, color = "white") +
  remove_y_axis
```
...
ggplot(mpg |> dplyr::count(class)) +
  aes(class, n, label=n) +
  geom_col() +
  geom_text(vjust = 0, nudge_y = 1, color = "black") +
  remove_y_axis
```

The problem with this approach is that if the nudge is too high it might run off the bar or chart.
...
This does the fixing of the label automatically, but not removing the y axis.
...
```{r}
# line to highlight the outliers above y=40 HORIZONTAL
p + geom_hline(yintercept=40, linetype="dashed")

# divide regions approximately by cyl VERTICAL
p + geom_vline(xintercept=c(2.6, 4.1), linetype="dashed")

# geom_abline DIAGONAL
p + geom_abline(intercept = 0, slope = 10, linetype = "dashed")
```
...
divisions <- c(2.6, 4.1)
df.rect <- data.frame(
  xmin = c(-Inf, divisions), # inf makes the rectangle cover the whole area
  xmax = c(divisions, Inf),
  ymin = -Inf,
  ymax = Inf,
  cyl = factor(c(4, 6, 8), levels=c(4, 5, 6, 8))
)

# plotting data
ggplot(mpg) +
  geom_rect(aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, fill=cyl),
            df.rect, alpha=.2, show.legend=FALSE) +
  scale_fill_discrete(drop=FALSE) +
  geom_point(aes(displ, hwy, color=factor(cyl))) +
  theme_bw()
```

The problem with defining rectangles with exact limits means that ggplot adds space around the data. You cannot cover the whole area of the graph if you do not remove this space (or add Inf).
...
Consult geom_segment for aesthetics. Must define x, y, xend or yend.

You can specify arrows.
...
ggplot(mpg) +
  aes(displ, hwy) +
  geom_point(data=annotation, color="yellow", size=3) +
  geom_point(aes(color=factor(cyl)))
```
...
```{r}
ottawa <- sf::st_sfc(sf::st_point(ottawa), crs=4326) # add coordinate reference system CRS, can get it from the map
...
To get the reference system of any object, we use:
