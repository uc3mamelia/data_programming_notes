...
label\_...() has many helpful functions
...
ggplot(mtcars) +
  aes(mpg, wt) +
  geom_point(aes(color=factor(cyl, levels=c(4, 5, 6, 7, 8)))) +
scale_color_discrete(name="Cylinders", drop=FALSE)
```
...
```{r}
p + 
  scale_x_continuous(transform = "hms")
```
...
ggplot(mtcars) +
  aes(log10(mpg), wt) + geom_point(aes(color = Cylinders))
```

Logarithmic vs base 10 scale

1.  Read the documentation for [`scale_x_continuous()`](https://ggplot2.tidyverse.org/reference/scale_continuous.html) and investigate how to do this. What is the default transformation? What does the `"reverse"` transformation do?
...
Reverses x axis, thereby reversing data
