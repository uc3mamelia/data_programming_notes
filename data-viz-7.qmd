...
Plots created with `coord_fixed()`, `coord_polar()` and `coord_sf()` cannot be resized to align with other plots. This happens frequently with maps. By default, `patchwork` aligns the other plots to them when possible.
...
```{r}
p_fixed + p1 + p2 + p3 + plot_layout(heights = 1)
...
```{r}
p1 + p2 + p3 + p4
p1 + p2 + p3 + p4 + plot_layout(guides="collect") # this has a bug, it doesn't play well with ggplot. But it is supposed to search through the plots to see if they have the same legends, but it doens't seem to be working
```
...
```{r}
p1 + p2 + (p3 + plot_layout(guides="keep")) + p4 + plot_layout(guides="collect") # this is keeping a legend for p3
```
...
p1 + p2 + p3 + p4 + guide_area() +
  plot_layout(design=layout, guides="collect") # we should only have one centered, but instead we have 2 because of the bug
```
...
p1 + table # adds a table to the graph
text + p1 # add text to the plot
```
...
# annotations with the layout
```

## **Insets**

`custom_annotation()` will also let you inset a graph with the grob. It is more flexible, but insets are easier.
...
```{r}
p1 + inset_element(p2, left=0.6, bottom=0.6, right=1, top=1, align_to="panel") # alignment to the panel
p1 + inset_element(p2, left=0.6, bottom=0.6, right=1, top=1, align_to="plot") # alignment to the plot
p1 + inset_element(p2, left=0.6, bottom=0.6, right=1, top=1, align_to="full") # alignment to the full plot
```
...
With `inset_element()` can do insets within the panel and also outside the panel.
...
p_zoom <- p1 +
  ...
  ) # this part zooms in on the graph

p1 +
  annotate("rect", xmin=xlim[1], xmax=xlim[2], ymin=ylim[1], ymax=ylim[2], fill=NA, color="black") +
