...
year_term_counts |>
  #we filter by our terms vector
  filter(term %in% c("peace", "win", "equality", "democracy")) |>
  ggplot(aes(year, count / year_total)) +
  geom_point() +
  geom_smooth() +
  facet_wrap(~ term, scales = "free_y") +
...
```{r}
library(gutenbergr)
nonfict <- gutenberg_download(c(1228, 205, 4300), mirror = "http://mirror.csclub.uwaterloo.ca/gutenberg/")
```

|  |  |  |  |
|------------------|------------------|------------------|------------------|
| **Gutenberg ID** | **Title** | **Author** | **Genre/Topic** |
| **1228** | *On the Origin of Species* | Charles Darwin | Science / Biology |
| **205** | *Walden* | Henry David Thoreau | Memoir / Philosophy |
| **4300** | *Ulysses* (Intro/Preface)\* | James Joyce / Various | Literary Criticism |
...
Sparsity in the non-fiction books is 56%, slightly higher but very similar to Austen's, so we can conclude that they had a similarly rich lexical selection.
...
If we set remove_separators as false, we include spaces as a token.
...
This doesn't work great because ". -" doesn't register as the end of a sentence.
...
We can see community is also including "Community", or "community's".
...
```{r}
realnews_dfm2 <- dfm(tokens_real_filtered)
set.seed(100)

textplot_wordcloud(realnews_dfm2, min_count = 6, random_order = FALSE,
                   rotation = .25, 
                   colors = RColorBrewer::brewer.pal(8,"Dark2"))
```

A word cloud with stemming is not useful because you need to see the whole word. However, lemmatization is super helpful in word clouds, because you don't want to see "university" and "universities".
...
It is impossible that 144 texts contain both features. It happens because we are measuring two different things: the 121 is the number of documents. The 144 is the number of matched tokens in each category (appearances). If a document contains several dictionary words, it is counted multiple times. We need to convert the counts into document counts.

```{r}
# copy from revised solution notebook
```

61 reception only, 12 both features, 48 rejection only.

This is more bias towards reception only. We should keep investigating. We should compare with the real news or look at the words in context.
...
```{r}
kwic(tokens(spam_corpus), "urgent", valuetype="regex", window=8)
```
